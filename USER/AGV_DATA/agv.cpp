...
uint8_t agv_buffer[2][8];
char agvRvBuff[AGV_RVSIZE]={0};
char agvBuff[AGV_RVSIZE]={0};
uint8_t agv_right[4];
uint8_t agv_left[4];
float turn_angle;
...
void read_agv_data()
{
    ...
    {
        agv_buffer[0][0] = agvBuff[1];
        agv_buffer[0][1] = agvBuff[0];
        agv_buffer[0][2] = agvBuff[3];
        agv_buffer[0][3] = agvBuff[2];
        agv_buffer[0][4] = agvBuff[5];
        agv_buffer[0][5] = agvBuff[4];
        agv_buffer[0][6] = agvBuff[7];
        agv_buffer[0][7] = agvBuff[6];
    }
    for(int i=0;i<4;i++)
    {
